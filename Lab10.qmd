...
### FINISH THE CODE HERE
cv_states <- merge(cv_states, state_pops, by = "state", all.x = TRUE)
```
...
District of Columbia is an extreme population-density outlier and should be excluded.
...
-   Create a scatterplot using `plot_ly()` representing `pop_density` vs. various variables (e.g. `cases`, `per100k`, `deaths`, `deathsper100k`) for each state on most recent date (`cv_states_today`)
    ...
    -   Use hover to identify any outliers.
        ...
        District of Columbia is the outlier that needs to be removed.
...
-   Explore the pattern between $x$ and $y$ using `geom_smooth()`
    ...
    The smoothed trend is flat, indicating very little systematic relationship between density and deaths per 100k at the single “today” after DC is removed.
...
-   Create one more line chart, for Florida only, which shows `new_cases` and `new_deaths` together in one plot. Hint: look for an `add_*()`
    -   Use hoverinfo to "eyeball" the approximate peak of deaths and peak of cases. What is the time delay between the peak of cases and the peak of deaths?
        ...
        From the two-line plot, peak deaths trail peak cases by about 3–4 weeks (18 to 28 days)
...
With raw counts the big population states dominate like CA, TX, FL and NY.

After normalization the smaller states with high incidence rates like LA, MS, TN stand out.
...
Hotspots in Alaska, Montana, Idaho, Wyoming etc are seen on Oct 15 2021, but in the most recent date the states are very light, which could be due to changes in reporting covid, better immunity or lack of testing. The national pattern is quite uniform.
